...
export const AdminMessage: React.FC<{ message: MessageModel, 
    ...
    function submitBtn() {
        // Check if the message id and response are not null
        if (props.message.id!== null && response!== '') {
            // If so, call the submitResponseToQuestion function with the message id and response
            props.submitResponseToQuestion(props.message.id, response);
            // Set the display warning to false
            setDisplayWarning(false);
        } else {
            // If not, set the display warning to true
